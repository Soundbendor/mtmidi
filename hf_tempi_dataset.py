...
class STHFTempiData(torch.utils.data.Dataset):
    def __init__(self, cur_df, embedding_type='mg_small_h', device='cpu', norm_labels = True, layer_idx=-1, class_binsize = 10, is_64bit = True, num_classes = -1, bpm_class_mapper = None, is_memmap = True, other_projdir = ''):
        self.seed = 5
    ...
    def __getitem__(self, idx):
        ...
        cur_arr = UD.get_data_vec_at_idx(cur_name, self.layer_idx, self.embedding_type, is_memmap = self.is_memmap, acts_folder = 'acts', dataset = 'tempos', to_torch = True, use_64bit = self.is_64bit, device = self.device, other_projdir = self.other_projdir)
