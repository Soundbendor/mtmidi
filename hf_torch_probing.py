...
if dataset =='tempi':
    ...
    seed = 42
    dataset_labels = pd.read_csv(csvfile)
    dataset_size = dataset_labels['name'].count()
    ...
    test_df, valid_df = train_test_split(
        hold_out_shuffled, train_size=0.5, random_state=seed
    ) 
...
def load_emb(df, layer_idx=-1):
	ret = None
	for i,x in enumerate(df['name']):
		fpath = os.path.join(data_folder, f'{x}.pt')
        cur_arr = torch.load(fpath, map_location=torch.device(device))
		if i == 0:
			if layer_idx >= 0:
				ret = cur_arr[layer_idx]
			else:
				ret = cur_arr
		else:
			if layer_idx >= 0:
				ret = torch.vstack((ret, cur_arr[layer_idx]))
			else:
				ret = torch.vstack((ret, cur_arr))
    return ret

def get_batch_idxs(emb, bs=64):
    idxs = random.sample( list(range(emb.shape[0])),min(bs, emb.shape[0]),)
    return idxs
 
def train_epoch(_model, tdf, _lossfn, _optimfn):
    _model.train(True)
    tot_loss = 0.
	epoch = (step * bs) / X_train.shape[0]
	if (self.cfg["max_num_epochs"] is not None and epoch > self.cfg["max_num_epochs"]):
		break
	_optimfn.zero_grad()
        _loss = None
