...
class STHFNotesData(TUD.Dataset):
    def __init__(self, cur_df, embedding_type = 'mg_small_h', device='cpu', layer_idx=-1, is_64bit = True, is_memmap = True, other_projdir = ''):
        ...
        # pitch classes are 0-indexed anyways
        self.pc_idx = self.data.select(['root_note_pitch_class']).to_numpy().flatten()
    ...
    def __getitem__(self, idx):
        ...
        cur_truth = cur_row[self.coldict['root_note_pitch_class']]
        cur_arr = UD.get_data_vec_at_idx(cur_name, self.layer_idx, self.embedding_type, is_memmap = self.is_memmap, acts_folder = 'acts', dataset = 'notes', to_torch = True, use_64bit = self.is_64bit, device = self.device, other_projdir = self.other_projdir)
