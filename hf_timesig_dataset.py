...
class STHFTimeSignaturesData(TUD.Dataset):
    def __init__(self, cur_df, embedding_type = 'mg_small_h', device='cpu', layer_idx=-1, is_64bit = True, save_ext = 'dat'):
        ...
        self.all_timesig = self.data.select(['time_signature']).to_numpy().flatten()
    ...
    def __getitem__(self, idx):
        ...
        if self.save_ext == 'dat':
            ...
            cur_arr =  UM.embedding_file_to_torch(self.embedding_type, acts_folder = 'acts', dataset='time_signatures', fname=cur_fname, layer_idx = self.layer_idx, device = self.device, use_64bit = self.is_64bit)
        else:
            cur_fname = f'{cur_name}.npy'
            cur_arr = UM.npy_to_torch(self.embedding_type, acts_folder = 'acts', dataset='time_signatures', fname=cur_fname, layer_idx = self.layer_idx, use_64bit = self.is_64bit, device = self.device)
